...
static  DATA_FRAME t_window[MAXSEQNO + 1];    
static  int trans_frame_state[MAXSEQNO + 1];   

static  int recv_frame_status[MAXSEQNO + 1];    
...
{   
    ...
    if(num_unack==(MAXSEQNO+1)/2){
        wait_timer = CNET_start_timer(EV_TIMER1, TIMEOUT, 0);
        CNET_disable_application(ALLNODES);
        return;
    }

    // if(trans_frame_state[next_frame%((MAXSEQNO+1)/2)] == ACK_WAIT)
    // {
    //     wait_timer = CNET_start_timer(EV_TIMER1, TIMEOUT, 0);
    //     CNET_disable_application(ALLNODES);
    //     return;
    ...
    next_frame = (next_frame + 1) % (MAXSEQNO + 1);
}
...
static EVENT_HANDLER(physical_ready)
{
    if(nodetype == RECEIVER)
    {
        ...
        int i;
        for(i = (last_ack_sent + 1) % (MAXSEQNO + 1); i != f.seq; i = (i + 1) % (MAXSEQNO + 1))
        {
    ...
    else
    {
        ...
        if(f.seq > 0)
        {
            ...
            int i;
            for(i = (last_ack_recv + 1)%(MAXSEQNO + 1); i != f.seq - 1; i = (i + 1) % (MAXSEQNO + 1))
...
static EVENT_HANDLER(wait)
{
    ...
    int i;
    for(i = (last_ack_recv + 1) % (MAXSEQNO + 1); i != next_frame; i = (i + 1) % (MAXSEQNO + 1))
...
static EVENT_HANDLER(record_result)
{
    if(nodetype == TRANSMITTER)
    {
        if(end_time == 0)
            end_time = nodeinfo.time_of_day.sec;
        FILE *f = fopen("results_selectiveReject", "a");
        fprintf(f, "%d %f\n", MAXSEQNO, packets_sent*1.0*DATA_FRAME_SIZE_MAX*8/(end_time - start_time));
