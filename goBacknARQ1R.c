...
static EVENT_HANDLER(app_ready)
{
   ...
   // CHECK(CNET_read_application(&dest, last_msg, &len));
   // CNET_disable_application(ALLNODES);
   ...
   if(nodetype == RECEIVER)
   {
       ...
       f.checksum  = 0;
        ...
        if(f.seq != frame_expected){
           ...
           send_ack_frame(frame_expected, REJ);
           return;
       }
       ...
       frame_expected=(frame_expected + 1)%(MAXSEQ + 1);
...
static EVENT_HANDLER(wait)
{
   ...
   int i;
   // next_frame=(last_ack+1)%(MAXSEQ + 1);
...
static EVENT_HANDLER(record_result)
{
   if(nodetype == TRANSMITTER)
   {
       if(end_time == 0)
           end_time = nodeinfo.time_of_day.sec;
       FILE *f = fopen("results_goBackN", "a");
